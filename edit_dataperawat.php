...
<body>
    ...
    <table class="table table-striped table-bordered table-hover">
     <?php
        $sqledit=mysqli_query($koneksi, "SELECT * FROM data_perawat WHERE id_perawat='$_GET[id]'");
        $e=mysqli_fetch_array($sqledit);
        ?>
        <h3>Edit Data Perawat</h3>
        <form method="post" action="">
            <input type="hidden" name="id_perawat" value="<?php echo $e['id_perawat']; ?>" />
            <table>

                <tr>
                    <td>Nama Perawat</td>
                    <td><input type="text" name="nama_perawat" value="<?php echo $e['nama_perawat']; ?>" /></td>
                </tr>
                <tr>
                    <td>Alamat</td>
                ...
                <tr>
                     <td><a href="dataperawat.php">Kembali ke Data Perawat</a></td>
                </tr>
            </table>
        </form>
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            if($nama==''){
                ...
                $edit = mysqli_query($koneksi, "UPDATE data_perawat SET nama_perawat='$nama' WHERE id_perawat='$id'");
                $edit = mysqli_query($koneksi, "UPDATE data_perawat SET alamat='$alamat' WHERE id_perawat='$id'");
                $edit = mysqli_query($koneksi, "UPDATE data_perawat SET no_tlp='$no_tlp' WHERE id_perawat='$id'");
                
                if(!$edit){
                    echo "Edit data gagal!!";
                }else{
                    echo "Data Berhasil Dirubah Silahkan Kembali ke Data Perawat";
                }
            }
        }
        ?>
